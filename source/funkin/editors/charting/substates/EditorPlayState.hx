...
class EditorPlayState extends MusicBeatSubstate
{
	...
	{
		Conductor.safeZoneOffset = (Preferences.data.safeFrames / 60) * 1000 * playbackRate;
	...
	override function update(elapsed:Float)
	{
		...
		var time:Float = CoolUtil.floorDecimal((Conductor.songPosition - Preferences.data.noteOffset) / 1000, 1);
	...
	function generateSong()
	{
		...
		var songSpeedType:String = Preferences.getGameplaySetting('scrolltype');
		switch(songSpeedType)
		{
			case "multiplicative":
				songSpeed = PlayState.SONG.speed * Preferences.getGameplaySetting('scrollspeed');
			case "constant":
	...
	{
		var strumLineX:Float = Preferences.data.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X;
	...
	{
		if(Preferences.data.noteOffset <= 0) {
			endSong();
		} else {
