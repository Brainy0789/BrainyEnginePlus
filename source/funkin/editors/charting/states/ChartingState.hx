...
class ChartingState extends MusicBeatState implements BrainyUIEventHandler.BrainyUIEvent
{
	...
	function addFileTab()
	{
		...
		var btn:BrainyUIButton = new BrainyUIButton(btnX, btnY, '  V-Slice to Psych...', function()
		{
			...
			{
				var chart:VSliceChart = cast TJSON.parse(fileDialog.data);
				...
				{
					var metadata:VSliceMetadata = cast TJSON.parse(fileDialog.data);
	...
	function updateNotesRGB()
	{
		...
		for (note in notes)
		{
			...
			{
				var data = core.NoteTypesConfig.loadNoteTypeData(note.noteType);
	...
	function loadCharacterFile(char:String):CharacterFile
	{
		if(char != null)
		{
			try
			{
				...
				#end
				return cast TJSON.parse(unparsedJson);
			}
			catch (e:Dynamic) {}
		}
		return null;
