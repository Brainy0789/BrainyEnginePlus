...
class LoadingState extends MusicBeatState
{
	...
	static function _startPool()
	{
		...
		if (Preferences.data.multiThreadedLoading)
	...
	public static function prepareToSong()
	{
		...
		new Future<Bool>(() -> {
			...
			try
			{
				...
				if (FileSystem.exists(moddyFile)) json = TJSON.parse(File.getContent(moddyFile));
				else json = TJSON.parse(File.getContent(path));
				#else
				json = TJSON.parse(Assets.getText(path));
				#end
	...
	inline private static function preloadCharacter(char:String, ?prefixVocals:String)
	{
		try
		{
			...
			#if MODS_ALLOWED
			var character:Dynamic = TJSON.parse(File.getContent(path));
			#else
			var character:Dynamic = TJSON.parse(Assets.getText(path));
			#end
