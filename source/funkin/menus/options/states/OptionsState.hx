...
typedef ButtonData = {
    var isButton:Bool;
    @:optional var stateToSwitch:String;
    @:optional var subState:String;
}

typedef OptionData =
{
    var name:String;
    @:optional var save:String; //optional so buttons dont have to map to a save
    @:optional var type:String;
    @:optional var desc:String;
    @:optional var parameters:Dynamic;

    @:optional var button:ButtonData;
    @:optional var isText:Bool; //if isText is enabled, object is turned into a simple text that displays desc 
}
...
typedef OptionsStateData = {
    var tabs:Array<OptionTab>;
}
...
class OptionsState extends MusicBeatState implements BrainyUIEvent
{
    ...
    function createExperimentsTab()
    {
        ...
        #if desktop
        mtl = new OptionSprite(objX, objY, 'Multi-threaded Loading', 'multiThreadedLoading', 'bool', "Improves loading times, but may crash the game on older PCs.");
        tab.add(mtl);
        objY += spacing;
        #end
    }
    ...
    public inline static function makeText(X, Y, Text):FlxText
        return new FlxText(X, Y, instance.box.width - X - 10, Text);
}